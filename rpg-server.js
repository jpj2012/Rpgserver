  ...
  ws.on("message", async raw => {
    ...
    switch(msg.type) {
      ...
      case "login": {
        ...
        let { data: player } = await supabase.from("rpg_players")
          .select("*").eq("id", msg.playerId).single().catch(()=>({data:null}));
        ...
        // Load characters
        const { data: chars } = await supabase.from("rpg_characters")
          .select("*").eq("player_id", msg.playerId);

        client.playerId = msg.playerId;
        ws.send(JSON.stringify({ type:"login_ok", player, characters: chars||[] }));
        break;
      }
      ...
      case "attack": {
        ...
        if (enemy.currentHp<=0) {
          ...
          for (const p of session.players) {
            ...
            await supabase.from("rpg_players").select("gold").eq("id",p.playerId).then(async ({data})=>{
              if(data) await supabase.from("rpg_players").update({ gold:(data.gold||0)+goldEarned }).eq("id",p.playerId);
            });
